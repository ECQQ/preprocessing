...
def get_dialogues_info(frame):
    ...
    for k in range(1, 6):
        ...
        partial['source_id'] = frame['ID Archivo']
    ...
    diag_groups = needs.groupby('source_id')
    needs['priority'] = [i for g, f in diag_groups for i in range(f.shape[0])]
    needs    = needs[['source_id', 'name', 'name_tokens',
...
def create_table_personal_needs(frame, indv_frame, indv_online_frame):
    ...
    need_diag['is_dialogue'] = np.ones(need_diag.shape[0])
    need_diag['is_online'] = np.zeros(need_diag.shape[0])

    need_ind['is_dialogue'] = np.zeros(need_ind.shape[0])
